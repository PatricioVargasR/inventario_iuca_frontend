  ...
  <div class="pagination">
    ...
    <div class="pagination-controls">
      ...
      <template v-for="p in pages" :key="p">
        <span v-if="p === '...'" class="page-btn" style="cursor:default;border:none;color:var(--gray-400)">…</span>
        <button v-else class="page-btn" :class="{ active: p === current }" @click="$emit('change', p)">{{ p }}</button>
      </template>
...
const from = computed(() => Math.min((props.current - 1) * props.perPage + 1, props.total))
...
const pages = computed(() => {
  ...
  if (c <= 4) return [1, 2, 3, 4, '...', tp]
  if (c >= tp - 3) return [1, '...', tp - 3, tp - 2, tp - 1, tp]
