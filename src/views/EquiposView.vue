<template>
  <div class="page-container">
    <PageHeader title="Equipos de cómputo" subtitle="Gestión del inventario de equipos">
      <template #actions>
    ...
    <div class="filter-bar">
      ...
      <div class="filter-group">
        ...
        <select v-model="filters.tipo_activo_id" class="form-select" @change="loadData">
          ...
          <option v-for="t in catalogos.tipos" :key="t.nombre_tipo" :value="t.nombre_tipo">
            {{ t.nombre_tipo }}
          </option>
        </select>
      </div>
      <div class="filter-group">
        ...
        <select v-model="filters.estado_id" class="form-select" @change="loadData">
          ...
          <option v-for="e in catalogos.estados" :key="e.nombre_estado" :value="e.nombre_estado">
            {{ e.nombre_estado }}
          </option>
        </select>
      </div>
      ...
      <table class="data-table">
        <thead>
          <tr>
            <th @click="toggleSort('id_activo')" class="sorted">
              <span class="sort-btn">
                ID
                <span class="sort-icon" :class="getSortClass('id_activo')">
            ...
            <th @click="toggleSort('tipo_activo')" class="sorted">
              <span class="sort-btn">
                Tipo
                <span class="sort-icon" :class="getSortClass('tipo_activo')">
                  <span class="arr-up"></span>
                  <span class="arr-down"></span>
                </span>
              </span>
        ...
        <tbody>
          <tr v-if="loading" class="loading-row">
            <td colspan="9"><span class="spinner"></span></td>
          </tr>
          ...
          <tr v-else v-for="eq in equipos" :key="eq.id_activo">
            ...
            <td>
              <TableActions :show-edit="authStore.canDo('computo', 'puede_actualizar')"
                :show-delete="authStore.canDo('computo', 'puede_eliminar')" @view="handleOpenDetail(eq)"
                @edit="handleOpenEdit(eq)" @delete="confirmDelete(eq.id_activo, eq)" />
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <BaseModal v-model="showDetail" title="Detalles del Equipo" size="lg">
      <template v-if="selected">
        <div class="detail-grid">
          ...
          <div class="detail-item">
            <label>Tipo</label><strong>{{ selected.tipo_activo }}</strong>
          </div>
          <div class="detail-item">
          ...
          <div class="detail-item">
            <label>Marca</label><strong>{{ selected.marca || "–" }}</strong>
          </div>
        ...
        <div v-if="selected.especificaciones" style="margin-top: 16px">
          ...
          <div class="specs-box">
            <div v-for="(spec, i) in selected.especificaciones.split('|')" :key="i" class="spec-item">
              {{ spec.trim() }}
            </div>
          </div>
        </div>
      ...
      <form id="equipoForm" @submit.prevent="handleSave" novalidate>
        ...
        <div class="form-grid">
          <div class="form-group">
            ...
            <select v-model="form.tipo_activo_id" class="form-select"
              :class="{ 'input-error': formErrors.tipo_activo_id }" required>
              <option value="">Seleccionar...</option>
              <option v-for="t in catalogos.tipos" :key="t.id_tipo_activo" :value="t.id_tipo_activo">
                {{ t.nombre_tipo }}
              </option>
            </select>
          ...
          <div class="form-group">
            ...
            <input v-model="form.nombre_activo" class="form-input" :class="{ 'input-error': formErrors.nombre_activo }"
        ...
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <input v-model="form.numero_serie" class="form-input" :class="{ 'input-error': formErrors.numero_serie }"
          ...
          <div class="form-group span-full">
            ...
            <textarea v-model="form.observaciones" class="form-textarea" placeholder="Observaciones relevantes..."
        ...
        <div v-if="form.tipo_activo_id && specSuggestions.length" class="spec-quick-chips">
          ...
          <button v-for="sug in specSuggestions" :key="sug.nombre" type="button" class="spec-chip-btn" :class="{
            used: form.especificaciones.some(
              (s) => s.nombre_especificacion === sug.nombre,
            ),
          }" @click="
            !form.especificaciones.some(
              (s) => s.nombre_especificacion === sug.nombre,
            ) &&
            form.especificaciones.push({
              nombre_especificacion: sug.nombre,
              valor_especificacion: '',
              _placeholder: sug.placeholder,
            })
            ">
            <svg v-if="
              form.especificaciones.some(
        ...
        <div v-for="(spec, i) in form.especificaciones" :key="i" class="spec-row" :class="`spec-row-${i}`">
          <div class="spec-nombre-wrapper">
            <input v-model="spec.nombre_especificacion" class="form-input"
              :class="{ 'input-error': specErrors[i]?.nombre }" placeholder="RAM, Procesador..."
              @focus="openSuggestions(i)" @input="openSuggestions(i)" @blur="closeSuggestions" />
            <div v-if="
              activeSuggestionIndex === i && filteredSuggestions(i).length
            " class="spec-suggestions-dropdown">
              <button v-for="sug in filteredSuggestions(i)" :key="sug.nombre" type="button" class="spec-suggestion-item"
                :class="{
                  used: form.especificaciones.some(
                    (s, idx) =>
                      idx !== i && s.nombre_especificacion === sug.nombre,
                  ),
          ...
          <div style="flex: 1">
            <input v-model="spec.valor_especificacion" class="form-input spec-value-input"
              :class="{ 'input-error': specErrors[i]?.valor }" :placeholder="spec._placeholder || 'Valor...'" />
          </div>
        ...
        <p v-if="formErrors.especificaciones" class="field-error" style="margin-top: 6px">
          {{ formErrors.especificaciones }}
        </p>
      </form>
    ...
    <!-- Confirm Delete -->
    <ConfirmDialog v-model="showConfirm" title="Eliminar Equipo"
      :message="`¿Estás seguro de eliminar '${toDelete?.nombre_activo}'? Esta acción no se puede deshacer.`"
      :loading="deleting" @confirm="handleDoDelete" @cancel="handleCancelDelete_" />
    ...
    <ConflictModal v-model="showConflictModal" entity-label="este equipo" @reload="handleConflictReload" />
  </div>
</template>
...
const specSuggestions = computed(() => {
  const tipo = catalogos.tipos.find(
    (t) => t.id_tipo_activo === form.tipo_activo_id,
  );
...
setLoadFn(loadData);
onMounted(() => {
  loadCatalogos(["tiposActivo", "estados", "usuarios"]);
  loadData();
});
</script>
