<template>
  <div class="page-container">
    ...
    <PageHeader title="Accesos al sistema" subtitle="Usuarios con credenciales para acceder al sistema">
      <template #actions>
        <button v-if="authStore.canDo('acceso', 'puede_crear')" class="btn btn-primary" @click="openCreate">
    ...
    <div class="filters-card">
      <div class="filters-row">
        ...
        <div class="filter-group">
          ...
          <select v-model="filters.area_id" class="form-select" @change="loadData">
            ...
            <option v-for="a in catalogos.areas" :key="a.nombre_area" :value="a.nombre_area">{{ a.nombre_area }}
            </option>
          </select>
        </div>
      </div>
      ...
      <div class="filter-chips-section" :class="{ disabled: filters.modulos.length === 0 }">
        ...
        <div class="chips-group">
          <label v-for="(value, key) in PERMISOS_DISPONIBLES" :key="key" class="chip-checkbox"
            :class="{ disabled: filters.modulos.length === 0 }">
            <input type="checkbox" :value="key" v-model="filters.permisos" @change="loadData"
      ...
      <table class="data-table">
        ...
        <tbody>
          ...
          <tr v-else-if="!items.length">
            <td colspan="7">
              <EmptyState text="No se encontraron accesos" icon="🔑" />
            </td>
          </tr>
          <tr v-else v-for="acc in items" :key="acc.id_acceso">
            <td><span style="font-family:var(--font-mono);font-size:12px;color:var(--gray-500)">{{ acc.id_acceso
            }}</span>
            </td>
            ...
            <td style="font-size:12px;color:var(--gray-400);font-family:var(--font-mono)">{{
              formatDate(acc.ultimo_acceso)
            }}</td>
            <td>
              <TableActions :show-edit="authStore.canDo('acceso', 'puede_actualizar')"
                :show-delete="authStore.canDo('acceso', 'puede_eliminar')"
                :delete-hidden="acc.id_acceso === currentUserId" @view="openDetail(acc)" @edit="openEdit(acc)"
    ...
    <BaseModal v-model="showDetail" title="Detalles del Acceso" size="lg">
      <template v-if="selected">
        ...
        <div class="detail-grid">
          ...
          <div class="detail-item"><label>Correo electrónico</label><strong>{{ selected.correo_electronico }}</strong>
          </div>
        ...
        <div class="detail-grid" style="grid-template-columns:1fr 1fr 1fr;">
          <div class="detail-item"><label>Último acceso</label><strong
              style="font-family:var(--font-mono);font-size:13px">{{ selected.ultimo_acceso || '–' }}</strong></div>
          <div class="detail-item"><label>Cuenta creada</label><strong>{{ selected.fecha_creacion?.slice(0, 10) || '–'
          }}</strong></div>
        </div>
        ...
        <table class="permissions-table">
          ...
          <tbody>
            <tr
              ...
              <td><span class="perm-icon" :class="{ active: p.leer }"><svg v-if="p.leer" width="16" height="16"
                    viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
                    <polyline points="20 6 9 17 4 12" />
                  </svg><svg v-else width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    stroke-width="3">
                    <line x1="18" y1="6" x2="6" y2="18" />
                    <line x1="6" y1="6" x2="18" y2="18" />
                  </svg></span></td>
              <td><span v-if="p.modulo !== 'historial'" class="perm-icon" :class="{ active: p.crear }"><svg
                    v-if="p.crear" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    stroke-width="3">
                    <polyline points="20 6 9 17 4 12" />
                  </svg><svg v-else width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    stroke-width="3">
              ...
              <td><span v-if="p.modulo !== 'historial'" class="perm-icon" :class="{ active: p.actualizar }"><svg
                    v-if="p.actualizar" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    stroke-width="3">
                    <polyline points="20 6 9 17 4 12" />
                  </svg><svg v-else width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    stroke-width="3">
              ...
              <td><span v-if="p.modulo !== 'historial'" class="perm-icon" :class="{ active: p.eliminar }"><svg
                    v-if="p.eliminar" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    stroke-width="3">
                    <polyline points="20 6 9 17 4 12" />
                  </svg><svg v-else width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    stroke-width="3">
    ...
    <!-- Form Modal -->
    <BaseModal v-model="showForm" :title="editMode ? 'Editar acceso' : 'Registro de acceso'"
      ...
      <form id="accesosForm" @submit.prevent="saveItem" novalidate>
        ...
        <div class="form-grid">
          <div class="form-group">
            ...
            <input v-model="form.nombre_usuario" class="form-input"
              :class="{ 'input-error': formErrors.nombre_usuario }" placeholder="Ej. Juan Pérez" />
            <span v-if="formErrors.nombre_usuario" class="field-error">{{ formErrors.nombre_usuario }}</span>
          </div>
          <div class="form-group">
            <label class="form-label">Correo electrónico <span class="required">*</span></label>
            <input v-model="form.correo_electronico" class="form-input"
              :class="{ 'input-error': formErrors.correo_electronico }" type="email" placeholder="usuario@iuca.edu.mx"
              :disabled="editMode" />
        ...
        <div class="form-grid">
          <div class="form-group">
            ...
            <div style="position:relative;">
              <input v-model="form.password" :type="showPass ? 'text' : 'password'" class="form-input"
                :class="{ 'input-error': formErrors.password }" placeholder="••••••••" style="padding-right:38px;"
                maxlength="10" />
              <button type="button"
          ...
          <div class="form-group">
            ...
            <input v-model="form.confirm_password" :type="showPass ? 'text' : 'password'" class="form-input"
              :class="{ 'input-error': formErrors.confirm_password }" placeholder="••••••••" maxlength="10" />
        ...
        <div style="margin-top:4px;">
          <div
            style="display:flex;align-items:center;justify-content:space-between;margin-top:20px;margin-bottom:12px;">
            <div class="section-title" style="margin:0;">Permisos del rol asignado</div>
            <label class="select-all-label">
              <input type="checkbox" :checked="todosSeleccionados" :indeterminate.prop="algunosSeleccionados"
                @change="toggleTodos" />
          ...
          <table class="permissions-table">
            ...
            <tbody>
              <tr v-for="p in [...permisosComoArray()].sort((a, b) => a.modulo.localeCompare(b.modulo))"
                :key="p.modulo">
                <td>{{ formatModuloNombre(p.modulo) }}</td>
                <td><span class="perm-icon" :class="{ active: p.puede_leer }"
                    @click="togglePermiso(p, 'puede_leer')"><svg v-if="p.puede_leer" width="16" height="16"
                      viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
                      <polyline points="20 6 9 17 4 12" />
                    </svg><svg v-else width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                      stroke-width="3">
                ...
                <td><span v-if="p.modulo !== 'historial'" class="perm-icon" :class="{ active: p.puede_crear }"
                    @click="togglePermiso(p, 'puede_crear')"><svg v-if="p.puede_crear" width="16" height="16"
                      viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
                      <polyline points="20 6 9 17 4 12" />
                    </svg><svg v-else width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                      stroke-width="3">
                ...
                <td><span v-if="p.modulo !== 'historial'" class="perm-icon" :class="{ active: p.puede_actualizar }"
                    @click="togglePermiso(p, 'puede_actualizar')"><svg v-if="p.puede_actualizar" width="16" height="16"
                      viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
                      <polyline points="20 6 9 17 4 12" />
                    </svg><svg v-else width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                      stroke-width="3">
                ...
                <td><span v-if="p.modulo !== 'historial'" class="perm-icon" :class="{ active: p.puede_eliminar }"
                    @click="togglePermiso(p, 'puede_eliminar')"><svg v-if="p.puede_eliminar" width="16" height="16"
                      viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
                      <polyline points="20 6 9 17 4 12" />
                    </svg><svg v-else width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                      stroke-width="3">
    ...
    <ConfirmDialog v-model="showConfirm" title="Eliminar acceso"
    ...
    <ConflictModal v-model="showConflictModal" entity-label="este acceso" @reload="handleConflictReload" />
  </div>
</template>
