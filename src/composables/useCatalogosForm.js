...
export function useCatalogosForm({
  ...
  const { formErrors, clearErrors, applyFieldErrors, setError } =
    useFormErrors();
  ...
  function validateForm() {
    ...
    if (!form.nombre?.trim()) {
      ...
      const maxLen =
        tipo === "tipo_activo" || tipo === "tipo_mobiliario" ? 30 : 50;
  ...
  async function tryAcquireLock(tab, id, ttl, tipo) {
    ...
    if (!result.success && result.locked) {
      ...
      concurrencyAlert.title =
        tipo === "eliminacion" ? "No se puede eliminar" : "Registro en uso";
      concurrencyAlert.message =
        tipo === "eliminacion"
          ? esEdicion
            ? `No puedes eliminar porque ${result.lockInfo.nombre_usuario} lo está editando`
            : `${result.lockInfo.nombre_usuario} ya está eliminando este registro`
          : esEdicion
            ? `${result.lockInfo.nombre_usuario} está editando este registro`
  ...
  async function saveItem() {
    ...
    try {
      if (editMode.value) {
        ...
        toast.success(
          `${capitalize(tabActual.value.labelSingular)} actualizado correctamente`,
          "Actualización exitosa",
        );
      } else {
        await catalogosApi[METODO_MAP[tipo].create](payload);
        toast.success(
          `${capitalize(tipoActual.value.labelSingular)} creado correctamente`,
          "Registro exitoso",
        );
      }
  ...
  async function handleConcurrencyRetry(items) {
    ...
    setTimeout(() => {
      ...
      if (concurrencyAlert.title === "No se puede eliminar")
        confirmDelete(activeTab.value, item);
